    ...
    if text != "":
        with st.spinner("🤖 Наш ИИ подбирает Вам навыки и тематику..."):
            if len(text) > 1500:
                text = text[:1500]
            if len(text) < 150:
                st.error("Ваша идея/описание слишком короткое :( Опишите подробнее, иначе мы не сможем правильно определить тематику. Введите не меньше 150 символов")
            else:
...
if st.button("Отправить", key="KEY_WORDS"):
    if news != "":
        with st.spinner("🤖 Наш ИИ подбирает оптимальные ключевые слова/фразы..."):
            ...
            if len(news) < 150:
                st.error(
                    "Текст слишком короткий :( Напишите подробнее, иначе выделение ключевых элементов не будет иметь смысла. Введите не меньше 150 символов")
            else:
    ...
    if news_theme != "":
        with st.spinner("🤖 Наш ИИ подбирает тему новости..."):
            ...
            if len(news_theme) < 150:
                st.error(
                    "Текст слишком короткий :( Напишите подробнее, иначе выделение тематики новости не будет работать.  Введите не меньше 150 символов")
            else:
